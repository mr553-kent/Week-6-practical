...
```{r}
# Add your code here!
reduce_fev_co <- subset(fev_data, select=-c(1,3,5))
head(reduce_fev_co)

#alternatively as below you could use the select function and assign it to a new data set in the environment
reduce2_fev <- select(fev_data, subjid, fev, sex, smoke)
head(reduce2_fev)

#or using the pipe opertator with not option
reduce3_fev <- fev_data %>% 
  select(-seqnbr,-age,-height)
head(reduce3_fev)
...
#working on the assumption that the average height of men should be higher than the females, i have grouped by sex and summarised with the mean height. From this data i would say that males=1 and females=0
```
...
fev_data %>% 
  group_by(smoke,sex) %>% 
  summarise(n=n(),mean(fev),mean(age),mean(height)
            )

#Looking at the other variables, this may be explained as the smoking group appears to be taller (hence large lung volume) and seem to be older (again potentially a larger lung volume as we are talking about children who are growing)

#There seems to be similar higher lung FEV for both males that smoke and females that smoke.

#??? doesnt quite make sense that we have a mean smoker age of 13 - this in not a healthy population
